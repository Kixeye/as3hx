...
class ClassParser {
    ...
    public static function parse(tokenizer:Tokenizer, types:Types, cfg:Config, path:String, filename:String, kwds:Array<String>, meta:Array<Expr>,isInterface:Bool) : ClassDef {
        ...
        var origName = tokenizer.id();
        var cname = origName;

        // War Commander-specific change: Rename classes so they don't conflict within a package.
        if (Lambda.has(["ELEMENT", "KEY", "NAMES"], cname)) {
            cname = cname + (Std.random(90000) + 10000);
        }
        ...
        var impl = [], extend = null, inits = [], initsMeta = [];
        ...
        while( true ) {
            ...
            if( ParserUtils.opt(tokenizer, TId("extends")) ) {
                if(!isInterface) {
                    ...
                    // Don't bother trying to extend classes from Object
                    switch (extend)
                    {
                        case TPath(a):
                            var ex = a.join(".");
    ...
    public static function parseVar(tokenizer, types:Types, cfg:Config, kwds, meta, condVars:Array<String>) : ClassField {
        ...
        var genType = ParserUtils.generateTypeIfNeeded(rv, cfg);
        if (genType != null)
