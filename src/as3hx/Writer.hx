...
class Writer
{
    ...
    public function new(config:Config, lookup:Lookup)
    {
        this.lvl = 0;
        this.cfg = config;
        this.lookup = lookup;
        ...
        this.typeRenameMap = new Map<String,String>();
        this.genTypes = [];
        this.imported = [];
    ...
    function writeField(field : ClassField, c : ClassDef)
    {
                ...
                if(isConst(field.kwds)) {
                    switch(field.kind) {
                        case FVar(t, val) if(val != null):
                            ...
                            switch(val) {
                                case EUnop(_, _, EConst(_)) | EConst(_): write("inline ");
                                default:
                            }
                        default:
                    }
        ...
        switch(field.kind) {
            case FVar(t, val):
                ...
                write("var " + lookup.getModifiedIdent(field.name));
            ...
            case FFun( f ):
                if (field.name == c.name)
                {
                    ...
                    var name = if (isGetter(field.kwds)) {
                        ...
                        lookup.getModifiedIdent(field.name);
                    }
            ...
            for (expr in arg.exprs) //for each expression within that argument
            {
                switch (expr) {
                    ...
                    case ECommented(s,b,t,e): // comment among arguments
                        // LS: This code seems to make it write the comment twice
                        /*if (pendingComma) {
                            pendingComma = false;
                            write(",");
                        }
    ...
    function writeEArray(e:Expr, index:Expr) {
        ...
        if(etype == "FastXML" || etype == "FastXMLList") {
            ...
            //write("/*!!!" + etype + "!!!*/");
            if((isMovieClip(etype) && cfg.reflectMovieClipFields) || isDynamicType(etype) || ((isArrayType(etype) || e.match(EIdent(_))) && itype != null && itype != "Int" && itype != "UInt")) {
    ...
    function writeInits(c : ClassDef) {
        ...
        writeECondCompEnd(getCondComp(c.initsMeta));
    }
    ...
    function getExprType(e:Expr):Null<String> {
        ...
        switch(e) {
            ...
            case ECall(e2, params): return getExprType(e2);
            case EField(e2, f):
                    ...
                    default:
                        if (t2 == null) {
                            switch (e2) {
                                case EIdent(t3):
                                    if (t3 == "this") {
                                        var fieldType = lookup.getField(thisName, f).type;
                                        return fieldType == null ? thisName : fieldType;
                                    } else if (t3.charAt(0).toUpperCase() == t3.charAt(0) && t3.charAt(0) != "_") {
                                        return lookup.getField(t3, f).type;
                                    } else {
                                        return lookup.getField(thisName, f).type;
                                    }
                                default:
                            }
                        } else {
                            var fieldType:String = lookup.getField(Std.string(t2), f).type;
                            if (fieldType != null) {
                                return fieldType;
                            }
                        }
            ...
            case EIdent(s):
                s = lookup.getModifiedIdent(s);
                if (context.exists(s)) {
                    return context.get(s);
                }
                return lookup.getField(thisName, s).type;
            ...
            case EBinop(_ => "/", _, _, _): return cfg.floatType;
            case EBinop(_, e1, e2, _) if(getExprType(e1) != cfg.floatType && getExprType(e2) != cfg.floatType): return "Int";
            case EConst(c):
                return switch(c) {
                    case CInt(_): "Int";
                    case CFloat(_): cfg.floatType;
    ...
    function writeModifiedIdent(s : String) {
        write(lookup.getModifiedIdent(s));
    }

    function writeResolvedIdent(s : String) {
        if (typeRenameMap.exists(s)) {
            s = typeRenameMap[s];
        }
        write(lookup.resolveFullIdent(thisName, s, context));
    }

    function formatFieldName(s : String) {
        // If a non-alphanumeric character appears, wrap the field in quotes.
    ...
    function writeExpr(?expr : Expr) : BlockEnd {
        ...
        switch(expr) {
            ...
            case EIdent(v): writeResolvedIdent(v);
            ...
            case ETernary(cond, e1, e2): writeETernary(cond, e1, e2);
            ...
            case EObject(fl):
                if (fl.empty()) {
                    ...
                    for (i in 0...length) {
                        var field = fl[i];
                        writeIndent(formatFieldName(field.name) + (cfg.spacesOnTypeColon ? " : " : ": "));
                        writeExpr(field.e);
    ...
    function writeEField(fullExpr:Expr, e:Expr, f:String):BlockEnd {
        ...
        //     write("." + f + ".innerData");
        //} else
        if (cfg.reflectMovieClipFields && t1 == null && isMovieClip(t2)) {
            write("Reflect.field(");
            writeExpr(e);
            write(", \"");
            write(f);
            ...
            switch(e) {
                case EField(e2, f2):
                    ...
                    if(getExprType(e2) == "FastXML")
                        inArrayAccess = true;
                case EIdent(v):
                    var modifiedV = lookup.getModifiedIdent(v);
                    if (modifiedV == "Int") {
                        if(f == "MAX_VALUE") {
                    ...
                    } else if (modifiedV == cfg.floatType) {
                        if(f == "MAX_VALUE") {
    ...
    inline function writeEVars(vars:Array<{name:String, t:Null<T>, val:Null<Expr>}>):BlockEnd {
        ...
        for(i in 0...vars.length) {
            ...
            write("var " + lookup.getModifiedIdent(v.name));
            writeVarType(v.t);
            if(rvalue != null) {
                write(" = ");
                if(isIntType(type)) {
                    ...
                    rvalue = switch (rvalue) {
                        case ETypedExpr(EBinop("||", s, d, false), _):
                            ETernary(s, s, d);
                        default:
                            rvalue;
                    }
    ...
    function writeECall(fullExpr:Expr, expr:Expr, params:Array<Expr>):BlockEnd {
        switch(expr) {
            case EField(expr, f):
                ...
                if (f == "push" && params.length > 1 && isArrayType(type)) {
                    for(it in params) {
                ...
                else if (f == "sort" && params.length == 0 && isArrayType(type)) {
                    writeExpr(expr);
                    write(".sort(Reflect.compare);");
                    writeExpr(ENL(null));
                    return None;
                }
            default:
        ...
        if (eCall != null) {
            switch(eCall) {
                ...
                case ECommented(s, b, t, e2):
                    ...
                    write("  // "+ s);
                    return None;
                case EField(_, _): return writeExpr(eCall);
                default:
                    ...
                    if(n.indexOf(".") == -1 && (c>=65 && c<=90)) {
                        ...
                        switch(n) {
                            ...
                            case "XML":
                                ...
                                write(")");
                            default:
                                var func = lookup.resolveFunction(thisName, expr, context);
                                if (func != null)
                                {
                                    handled = false;
                                } else {
        ...
        if(!handled) {
            ...
            for(i in 0...params.length) {
                ...
                var funcArg = func != null ? func.args[i] : null;
                if (funcArg != null && funcArg.t != null) {
                    switch(funcArg.t) {
                        case TPath(p): requiredType = lookup.getModifiedIdent(p[0]);
                        default:
                    }
                }
                ...
                switch(param) {
                    ...
                    default:
                        var paramType = getExprType(param);
                        if (requiredType != null && paramType != null && requiredType != paramType) {
                            if (requiredType == "Int" && paramType == cfg.floatType) {
                                write("Std.int(");
                                writeExpr(param);
                                write(")");
                            } else {
    ...
    inline function writeETernary(cond:Expr, e1:Expr, ?e2:Expr) {
        //write("");
        var rb = rebuildIfExpr(cond);
        if(rb != null) {
            writeExpr(rb);
    ...
    inline function writeEWhile(cond:Expr, e:Expr, doWhile:Bool):BlockEnd {
        ...
        if (doWhile) {
            ...
            switch (cond) {
            case EUnop("--", false, _):
                writeExpr(EBinop(">=", cond, EConst(CInt("0")), false));
            default:
                writeExpr(cond);
            }
            writeCloseStatement();
        ...
        if(!isWhileLoop) {
            ...
            switch(conds[0]) {
                case EBinop(op, e1, e2, nl):
                    switch(op) {
                        case "<=": // Limit must be incremented by 1 due to range (...) upper bound exclusivity.
                            switch(e2) {
                                case EConst(CInt(v)):
                                    var upperBound = Std.string(Std.parseInt(v) + 1);
                                    writeExpr(EConst(CInt(upperBound)));
                                case _:
    ...
    inline function writeEForEach(ev:Expr, e:Expr, block:Expr):BlockEnd {
        ...
        var regexpObjMap:EReg = ~/^haxe.ds.ObjectMap<([^,]*, *)?(.*)>$/;
        var regexpArray:EReg = ~/^Array<(.*)>$/;
        var assumeDynamic:Bool = false;
        var inferredType:String = null;
        if(varName == null) {
            ...
            inferredType = regexpMap.matched(1);
        } else if (t != null && regexpObjMap.match(t)) {
            inferredType = regexpObjMap.matched(1);
        } else if (t != null && regexpArray.match(t)) {
            inferredType = regexpArray.matched(1);
        } else {
        ...
        if (inferredType != null) {
            if (cfg.debugInferredType) {
                write("/* inferred type: " + inferredType + " */" );
            }
            context.set(varName, inferredType);
        }
        ...
        inArrayAccess = old;
        writeCloseStatement();
        var formattedBlock = formatBlockBody(block);
        if (assumeDynamic) {
            var evt = getExprType(ev);
            formattedBlock.insert(0, ENL(EVars([
                {
                    name: varName,
                    t: evt != null ? TPath([evt]) : null,
                    val: ETypedExpr( ECall( EField(EIdent("Reflect"), "field"), [e, EIdent("key")] ), TPath([inferredType]) )
                }
            ])));
        }
        var result = writeExpr(EBlock(formattedBlock));
        closeContext();
        return result;
    }
    ...
    inline function writeEForIn(ev:Expr, e:Expr, block:Expr):BlockEnd {
        openContext();
        var etype = getExprType(e);
        var mapRegexp:EReg = ~/^Map<([^,]*)?,?.*>$/;
        var arrayRegexp:EReg = ~/^Array<([^,]*)?,?.*>$/;
        var isMap:Bool = etype != null && mapRegexp.match(etype);
        var isArray:Bool = etype != null && arrayRegexp.match(etype);
        ...
        switch(ev) {
            case EVars(vars):
                if(vars.length == 1 && vars[0].val == null) {
                    write(vars[0].name);
                    if (!isMap || mapRegexp.matched(1) == null) {
                        context.set(vars[0].name, "String");
                    } else if (isIntType(mapRegexp.matched(1))) {
                        context.set(vars[0].name, "Int");
                    } else {
                        context.set(vars[0].name, mapRegexp.matched(1));
                    }
                    if (cfg.debugInferredType) {
    ...
    function writeEBinop(op:String, e1:Expr, e2:Expr, newLineAfterOp:Bool):BlockEnd {
        if(op == "as") {
                ...
                case EIdent(s):
                    switch(s) {
                        ...
                        default:
                            ...
                            write(", ");
                            switch(e2) {
                                case EIdent(s): writeResolvedIdent(s);
            ...
            if (eBinop == null) {
                switch (e2) {
                    case EBinop("||", s1, s2, _):
                        return writeExpr(EBinop(op, e1, ETernary(s1, s1, s2), newLineAfterOp));
                    default:
                }
            }
    ...
    inline function writeEUnop(op:String, prefix:Bool, e:Expr):BlockEnd {
        ...
        var rebuiltExpr = switch(e) {
            case ETernary(cond, lval, rval):
                var t = getExprType(cond);
                if (t == "String" && op == "!") {
                    var cond1 = EBinop("==", cond, EIdent("null"), false);
                    var cond2 = EBinop("==", EField(cond, "length"), EConst(CInt("0")), false);
                    var both = EParent(EBinop("||", cond1, cond2, false));
                    ETernary(both, lval, rval);
                } else {
                    null;
                }
            default: null;
        ...
        if (rebuiltExpr != null) {
            writeExpr(rebuiltExpr);
        } else if((isIntType(type) || type == "UInt") && op == "!") {
    ...
    inline function needCastToInt(e:Expr):Bool {
        ...
        return switch(e) {
            ...
            case EIdent(_) | EConst(_): getExprType(e) == cfg.floatType;
            case EField(_, _): getExprType(e) == cfg.floatType;
            case EParent(e): needCastToInt(e);
            case ECall(e, _):
                var func = lookup.resolveFunction(thisName, e, context);
                var retType = null;
                if (func != null && func.ret != null) {
                    retType = getExprType(func.ret.exprs[0]);
                }
                retType == cfg.floatType;
            case _: false;
        }
    }
    ...
    function writeCastToFloat(e:Expr) {
        var type = getExprType(e);
        if (type != cfg.floatType && type != "Int") {
        ...
        switch(e) {
            case EArray(a, i):
                ...
                if (atype != null) {
                    ...
                    // If we don't know what the expression type is, assume it's a Dynamic.
                    writeExpr(ECall(EField(EIdent("Reflect"), "deleteField"), [a, i]));
                }
            default:
                addWarning("EDelete");
    ...
    function rebuildIfExpr(e:Expr) : Expr {
        var isNumericType = function(s) {
            return (s == cfg.floatType || isIntType(s) || s == "UInt");
        }
        switch(e) {
            ...
            case EIdent(id):
                ...
                if (t == "Int" || t == "Uint") {
                    var nullCheck = EBinop("!=", e, EIdent("null"), false);
                    var zeroCheck = EBinop("!=", e, EConst(CInt("0")), false);
                    return EParent(EBinop("&&", nullCheck, zeroCheck, false));
                }
                else if (t == cfg.floatType) {
                    var nullCheck = EBinop("!=", e, EIdent("null"), false);
                    var zeroCheck = EBinop("!=", e, EConst(CInt("0")), false);
                    var nanCheck = EUnop("!", true, ECall(EField(EIdent("Math"), "isNaN"), [e]));
                    return EParent(EBinop("&&", nullCheck, EBinop("&&", zeroCheck, nanCheck, false), false));
                } else if (t == "String") {
                    var nullCheck = EBinop("!=", e, EIdent("null"), false);
                    var emptyCheck = EBinop("!=", e, EConst(CString("")), false);
                    return EParent(EBinop("&&", nullCheck, emptyCheck, false));
                } else {
            ...
            case EUnop(op, prefix, e2):
                ...
                if(op == "!") {
                    ...
                    if (t == "String" || t == "Int" || t == cfg.floatType) return EUnop(op, prefix, r2);
                    var f:Expr->Expr = null;
            ...
            case EField(e2, f):
                if (f == "length") return EBinop("!=", e, EConst(CInt("0")), false);
                return EBinop("!=", e, EIdent("null"), false);
            case ENL(e):
    ...
    function rebuildCallExpr(fullExpr : Expr, expr : Expr, params : Array<Expr>) : Expr {
        ...
        switch (expr) {
            case EField(e, f):
                if(f == "hasOwnProperty") {
                    var type = getExprType(e);
                    if (type != null && (type.startsWith("haxe.ds.ObjectMap") || type.startsWith("Map"))) {
                        var rebuiltExpr = EField(e, "exists");
                        result = ECall(rebuiltExpr, params);
                    } else {
                        if (type == "FastXML" || type == "FastXMLList") {

                            switch(params[0]) {
                                case EConst(CString(s)):
                                    if (s.length > 1 && s.charAt(0) == "@") {
                                        result = EField(EField(e, "has"), s.substr(1));
                            ...
                            result = ECall(EField(EIdent("Reflect"), "hasField"), [e].concat(params));
                        }
                    }
                }
                else if (f == "children") {
                    var type = getExprType(e);
                    if (cfg.useFastXML && type == "FastXML" && params.length == 0) {
                        result = EField(e, "elements");
                    }
                }
                else if (f == "replace") {
                    var type = getExprType(e);
                ...
                else if((f == "min" || f == "max") && params.length <= 2 && cfg.floatType != null) {
                    if(getIdentString(e) == "Math") {
                        result = ECall(EField(EIdent(cfg.floatType), f), params);
                    }
                }
                else if((f == "min" || f == "max") && params.length > 2) {
                    if(getIdentString(e) == "Math") {
                        if (cfg.floatType != null) {
                            e = EIdent(cfg.floatType);
                        }
                    ...
                    if(getExprType(e) == cfg.floatType) {
                ...
                else if (f == "hash" && getIdentString(e) == "MD5") {
                    result = ECall(EField(EIdent("Md5"), "encode"), params);
                }
            case EParent(e):
            ...
            case "||=":
                ...
                if(type != null) {
                    var cond = switch(type) {
                        case "Bool": lvalue;
                        case "Int" | "UInt" | "Float" | "KxFloat" | _: rebuildIfExpr(lvalue);
                    }
    ...
    static inline function isMovieClip(s:String):Bool return s == "MovieClip";

    function isMapType(s:String):Bool {
        if (s == null) {
            return false;
        }
        if (s.startsWith("Map")) {
        ...
        if (s.startsWith("haxe.ds.ObjectMap")) {
            return true;
        }
        if (cfg.openflDictionary && s.startsWith("Dictionary")) {
            return true;
        }
        return false;
    }
    ...
    function tstring(t : T, isNativeGetSet:Bool = false, fixCase:Bool = true) : String {
        ...
        return switch(t) {
            ...
            case TDictionary(k, v): cfg.openflDictionary ? "Dictionary" : "haxe.ds.ObjectMap<" + tstring(k) + ", " + tstring(v) + ">";
    ...
    public function process(program : Program, writer : Output):Map<String, Bool> {
        ...
        for (type in program.typesDefd) {
            this.typeRenameMap[type.origName] = type.name;
