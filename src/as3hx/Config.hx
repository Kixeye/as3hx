...
class Config {
    ...
    /** Transform Dictionary.<Key, Value> to Map **/
    public var openflDictionary : Bool;
    ...
    public var reflectMovieClipFields : Bool;

    /**
     * configurable thread poolsize, defaults to 4
     */
    ...
    public static function usage() {
        ...
        println("Kixeye specific version. Do not use willy nilly.");
    ...
    function processCommandLine() {
        ...
        src = Utils.directory(arg);
        dst = Utils.directory(args.shift(), "./out");
    }
    ...
    */
    ...
    public function fromXml(f:Fast) {
        for(el in f.elements) {
            ...
            case "openflDictionary":    setBoolField(el, false);
            case "useFastXML":          setBoolField(el, true);
            case "useCompat":           setBoolField(el, true);
            case "debugInferredType":   setBoolField(el, false);
            case "importPaths":         setImportPaths(el, []);
            case "threadPoolSize":      setIntField(el, 4);
            case "floatType":           setCharField(el, "Float");
            case "reflectMovieClipFields": setBoolField(el, false);
            default:
    ...
    function setIntField(f:Fast, defaultVal:Int) {
        var val = defaultVal;
        if(f.has.value) {
            val = Std.parseInt(f.att.value);
        }
        Reflect.setField(this, f.name, val);
    }
...
'<as3hx>
    ...
    <openflDictionary value="false" />
    <verifyGeneratedFiles value="false" />
    <useFastXML value="true" />
    <useCompat value="true" />
    <debugInferredType value="false" />
    <postProcessor value="" />
    <threadPoolSize value="4" />
    <importPaths></importPaths>
    <floatType value="Float"/>
    <reflectMovieClipFields value="false"/>
</as3hx>';
    }
}
